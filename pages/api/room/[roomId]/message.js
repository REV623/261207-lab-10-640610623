...
export default function roomIdMessageRoute(req, res) {
  if (req.method === "GET") {
    ...
    if(roomIdx === -1) return res.status(404).json({ok: false, message: "Invalid room id"})
    ...
    if(typeof(text) !== "string")  return res.status(400).json({ok: false, message: "Invalid text input"})
    //create new id
    const newId = uuidv4();
